...
![](img/image-20220202111056036-16437714601701.png#id=wQz13&originalType=binary&ratio=1&rotation=0&showTitle=false&status=done&style=none&title=)
...
<a name="UserDetailServiceImpl"></a>
...
<a name="JwtAuthenticationTokenFilter"></a>
...
![](img/image-20220208214106296.png#id=Fxx0J&originalType=binary&ratio=1&rotation=0&showTitle=false&status=done&style=none&title=)
...
![](img/image-20220227224537701.png#id=C6jya&originalType=binary&ratio=1&rotation=0&showTitle=false&status=done&style=none&title=)

![](img/image-20220227224443813.png#id=qhrhJ&originalType=binary&ratio=1&rotation=0&showTitle=false&status=done&style=none&title=)

秘钥

![](img/image-20220228230512598.png#id=mi8mw&originalType=binary&ratio=1&rotation=0&showTitle=false&status=done&style=none&title=)

![](img/image-20220228230933808.png#id=Yzs1y&originalType=binary&ratio=1&rotation=0&showTitle=false&status=done&style=none&title=)
...
![](img/image-20220313133714102.png#id=hqWo0&originalType=binary&ratio=1&rotation=0&showTitle=false&status=done&style=none&title=)
...
① 使用@EnableScheduling注解开启定时任务功能
	...
	我们可以在配置类上加上[@EnableScheduling ](/EnableScheduling ) 
...
② 确定定时任务执行代码，并配置任务执行时间
	...
	使用@Scheduled注解标识需要定时执行的代码。注解的cron属性相当于是任务的执行时间。目前可以使用 0/5 _ _ _ _ ? 进行测试，代表从0秒开始，每隔5秒执行一次。
...
如上我们用到的 0/5 _ _ _ _ ? *，cron表达式由七部分组成，中间由空格分隔，这七部分从左往右依次是：
...
0 0 0 LW * ?<br />LW可以连起来用，表示每月最后一个工作日，即每月最后一个星期五
...
1.代码变，文档变。只需要少量的注解，Swagger 就可以根据代码自动生成 API 文档，很好的保证了文档的时效性。<br />2.跨语言性，支持 40 多种语言。<br />3.Swagger UI 呈现出来的是一份可交互式的 API 文档，我们可以直接在文档页面尝试 API 的调用，省去了准备复杂的调用参数的过程。
...
#### 4.3.2 启用Swagger2
	...
	在启动类上或者配置类加 [@EnableSwagger2 ](/EnableSwagger2 ) 注解 
...
#### 4.3.3 测试
	...
	访问：[http://localhost:7777/swagger-ui.html](http://localhost:7777/swagger-ui.html)  注意其中localhost和7777要调整成实际项目的域名和端口号。
...
<a name="18ec296e"></a>
##### 4.4.2.1 接口描述配置[@ApiOperation ](/ApiOperation ) 
...
[@ApiImplicitParam ](/ApiImplicitParam ) 用于描述接口的参数，但是一个接口可能有多个参数，所以一般与 [@ApiImplicitParams ](/ApiImplicitParams ) 组合使用。 
...
<a name="fe5610fd"></a>
##### 4.4.3.1 实体的描述配置[@ApiModel ](/ApiModel ) 
...
<a name="f7dfee1f"></a>
##### 4.4.3.2 实体的属性的描述配置[@ApiModelProperty ](/ApiModelProperty ) 
...
<a name="SecurityConfig-2"></a>
##### SecurityConfig

之前已经复制过了

<a name="SystemLoginServiceImpl"></a>
...
<a name="UserDetailServiceImpl-1"></a>
##### UserDetailServiceImpl

复用原来的即可

<a name="LoginUser-1"></a>
##### LoginUser
...
#### 5.9.2 技术方案
 ...
    [https://easyexcel.opensource.alibaba.com/docs/current/quickstart/write#示例代码-1](https://easyexcel.opensource.alibaba.com/docs/current/quickstart/write#%E7%A4%BA%E4%BE%8B%E4%BB%A3%E7%A0%81-1)
